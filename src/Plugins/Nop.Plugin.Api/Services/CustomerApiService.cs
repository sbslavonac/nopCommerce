...
namespace Nop.Plugin.Api.Services
{
    public class CustomerApiService : ICustomerApiService
    {
        ...
        public async Task<IList<CustomerDto>> GetCustomersDtosAsync(DateTime? createdAtMin = null,
        ...
        public async Task<int> GetCustomersCountAsync()
        {
            ...
            return _customerRepository.Table.Count(customer => !customer.Deleted
                                     && (customer.RegisteredInStoreId == 0 || customer.RegisteredInStoreId == id));
        }
        ...
        public async Task<CustomerDto> GetCustomerByIdAsync(int id, bool showDeleted = false)
        {
            ...
            if (customerAttributeMappings.Count > 0)
            {
                ...
                var defaultStoreLanguageId = GetDefaultStoreLangaugeIdAsync();
        ...
        private async Task<int> GetDefaultStoreLangaugeIdAsync()
        {
            ...
            if (defaultLanguageId == 0)
            {
                ...
                var storeLanguages = await allLanguages.WhereAwait(async l =>
                   await _storeMappingService.AuthorizeAsync(l, (await _storeContext.GetCurrentStoreAsync()).Id)).ToListAsync();
        ...
        private async Task SetNewsletterSubscriptionStatusAsync(IEnumerable<CustomerDto> customerDtos)
        {
        ...
        private async Task<IEnumerable<string>> GetAllNewsletterCustomersEmailsAsync()
        {
            var id = (await _storeContext.GetCurrentStoreAsync()).Id;
