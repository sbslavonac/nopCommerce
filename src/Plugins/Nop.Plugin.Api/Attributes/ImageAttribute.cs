...
namespace Nop.Plugin.Api.Attributes
{
    public class ImageValidationAttribute : BaseValidationAttribute
    {
        ...
        public override async void Validate(object instance)
        {
            ...
            if (imageSrcSet || imageAttachmentSet)
            {
                ...
                if (_errors.Count == 0)
                {
                  ...
                  await  ValidatePictureBiteArrayAsync(imageBytes, mimeType);
                }
        ...
        private async Task ValidatePictureBiteArrayAsync(byte[] imageBytes, string mimeType)
        {
            if (imageBytes != null)
            {
                try
